...
class GoogleEntity:
    ...
    async def sync_serialize(self, agent_user_id):
        ...
        domain = state.domain
        device_class = state.attributes.get(ATTR_DEVICE_CLASS)
        manufacturer = entity_config.get(CONF_MANUFACTURER, "Home Assistant")
        ...
        model = entity_config.get(CONF_MODEL)
        if model:
            device["deviceInfo"]["model"] = model

        hardware = entity_config.get(CONF_HARDWARE_VERSION)
        if hardware:
            device["deviceInfo"]["hwVersion"] = hardware

        software = entity_config.get(CONF_SOFTWARE_VERSION)
        if software:
            device["deviceInfo"]["swVersion"] = software
