...
class SecuritySystem(HomeAccessory):
    ...
    def __init__(self, *args):
        ...
        self.char_status_fault = serv_security_system.configure_char(
            CHAR_STATUS_FAULT, value=0,
        )
        self.char_status_tampered = serv_security_system.configure_char(
    ...
    def async_update_state(self, new_state):
        ...
        faulted_zone = attrs.get("faulted_zone")
        status_fault = True if faulted_zone else False
        if self.char_status_fault.value != status_fault:
            self.char_status_fault.set_value(status_fault)

        tampered_zone = attrs.get("tampered_zone")
        status_tampered = True if tampered_zone else False
        if self.char_status_tampered.value != status_tampered:
            self.char_status_tampered.set_value(status_tampered)
