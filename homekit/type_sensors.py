...
class AirQualitySensor(HomeAccessory):
    ...
    def __init__(self, *args):
        ...
        super().__init__(*args, category=CATEGORY_SENSOR)
        state = self.hass.states.get(self.entity_id)
        serv_air_quality = self.add_preload_service(SERV_AIR_QUALITY_SENSOR)
...
class BinarySensor(HomeAccessory):
    ...
    def __init__(self, *args):
        ...
        service = self.add_preload_service(service_char[0], chars)
    ...
    def async_update_state(self, new_state):
        ...
        if self.support_active:
            ...
            if isinstance(active, bool) and self.char_active.value != active:
                self.char_active.set_value(active)

        if self.support_low_battery:
            low_battery = new_state.attributes.get(ATTR_LOW_BATTERY)
            if isinstance(low_battery, bool) and self.char_low_battery.value != low_battery:
        ...
        if self.support_tamper_detected:
            ...
            if isinstance(tamper_detected, bool) and self.char_tampered.value != tamper_detected:
                self.char_tampered.set_value(tamper_detected)

        if self.support_tampered:
            tampered = new_state.attributes.get(ATTR_TAMPERED)
            if isinstance(tampered, bool) and self.char_tampered.value != tampered:
