...
NEST_CAMERA_SCHEMA = CAMERA_SCHEMA.extend(
    {
        vol.Optional(CONF_SERVICE_NAME_PREFIX): cv.string,
    }
)
...
def validate_entity_config(values):
    ...
    for entity_id, config in values.items():
        ...
        if device:
            if device == DEVICE_NEST_CAMERA:
                config = NEST_CAMERA_SCHEMA(config)
...
class HomeKitSpeedMapping:
    ...
    def __init__(self, speed_list):
        ...
        if speed_list[0] != fan.SPEED_OFF:
            _LOGGER.warning(
                f"{speed_list} does not contain the speed setting {fan.SPEED_OFF} as its first element. Assuming that {speed_list[0]} is equivalent to 'off'",
            )
